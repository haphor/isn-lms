			...
			if ($topics->have_posts()){
				while ($topics->have_posts()){ $topics->the_post();
                    ...
                    <div class="tutor-course-topic tutor-topics-in-single-lesson <?php if($index == 1) echo "tutor-active"; ?>">
                        ...
                        <ul class="sub-courses d-flex flex-column flex-wrap" style="<?php echo $index > 1 ? 'display: none' : ''; ?>">
							...
							if ($lessons->have_posts()){
								while ($lessons->have_posts()){ $lessons->the_post();
                                    ...
                                    <li class="d-flex flex-row justify-content-between">
                                        <?php
                                        $lesson_title = "<a href='".get_the_permalink()."'> ";
                                        if (has_post_thumbnail()){
                                            $thumbnail_url = get_the_post_thumbnail_url(get_the_ID(), 'thumbnail');
                                            $lesson_title .= "<span><i style='background:url({$thumbnail_url})' class='tutor-lesson-thumbnail-icon $lesson_icon'></i>";
                                        }else{
                                            $lesson_title .= "<span><i class='$lesson_icon'></i>";
                                        }

                                        if ($is_enrolled){
                                            $lesson_title .= get_the_title() .'</span>';
